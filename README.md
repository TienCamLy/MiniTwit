...
- **Flow**:
  ...
  - To increase visibility, we have also introduced a log (`log.md` in the root path) where we write down design decisions made during development each week. A GitHub webhook has been added to our main communication channel, so that all members get notified whenever a change to the repository has been made.
